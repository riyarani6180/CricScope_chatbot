    ...
    if analyze:
        ...
        with st.spinner(""):
            ...
            st.session_state.last_prediction = {
                    ...
                    "loss_probability": round(lose * 100)
                }
...
momentum_score = round(max(min((crr - rrr) * 10, 100), -100), 2)

if momentum_score > 5:
    momentum_text = f"{batting_team} dominating the momentum"
    momentum_color = "#22c55e"
elif momentum_score > 0:
    momentum_text = f"{batting_team} slightly ahead in momentum"
    momentum_color = "#eab308"
else:
    momentum_text = f"{bowling_team} gaining control of the match"
    momentum_color = "#ef4444"

# Pressure calculation
pressure_index = round((rrr / crr), 2) if crr > 0 else 0

# Turning point detection
if wickets >= 7:
    turning_point = "Heavy wicket collapse detected"
elif rrr > 12:
    turning_point = "Required run rate pressure increasing rapidly"
elif crr > rrr:
    turning_point = "Batting side maintaining healthy chase momentum"
else:
    turning_point = "Match finely balanced"

match_status = "INTENSE"

if win > 0.8:
    match_status = "DOMINATING"
elif lose > 0.8:
    match_status = "DEFENDING"
elif abs(win - lose) < 0.1:
    match_status = "THRILLER"
    
# AI generated insight
if wickets >= 7 and rrr > 12:
    ai_summary = f"{bowling_team} have created massive pressure with crucial breakthroughs."

elif crr > rrr and wickets <= 3:
    ai_summary = f"{batting_team} are pacing the chase efficiently with wickets in hand."

elif abs(win - lose) < 0.1:
    ai_summary = "The contest is extremely close with momentum shifting every over."

elif win > 0.75:
    ai_summary = f"{batting_team} are in a commanding position with strong win probability."

elif lose > 0.75:
    ai_summary = f"{bowling_team} have taken strong control of the match situation."

else:
    ai_summary = "The game remains highly competitive with momentum shifting dynamically."
    ...
    st.markdown(f"""
        ...
        ">
            ...
            <div style="margin-top:20px;">
                ...
                ">
                    <div style="
                        width:{min(abs(momentum_score)*10,100)}%;
                        height:100%;
...
with insight_col2:
    st.markdown(f"""
        ...
        ">
            ...
            ">
                <div>
                    <div style="font-size:10px;color:rgba(212,175,55,0.4);">
                        Match Status
                    </div>
